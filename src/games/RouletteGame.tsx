...
const RouletteGame = () => {
    ...
    const { soundVolume, showGameAnimation, gameHotKeyEnabled, showGameInfo, showHotkeyPanel } = useSelector((state) => state.setting);
    ...
    const startBet = async () => {
        setLoading(true);
        setServerSeed("");
        const { data } = await axios.post("/roulette/create", { clientSeed: clientSeed })
        if (data.status) {
    ...
    return <div className="flex w-full justify-center mt-5 items-center flex-col">
                ...
                <div className="flex flex-col w-full h-full">
                    {(!isMobile) && < div className="flex justify-center p-2 w-full">
                        <div className=" md:w-[30%] w-[60%] relative" style={{ aspectRatio: "1/1", background: "linear-gradient(#0c3a6d 0%, rgba(49, 93, 207, 0) 100%)", borderRadius: "50% 50% 0 0" }}>
                            ...
                            <div className={`absolute transition-opacity ${showResult ? "opacity-95" : "opacity-0"} duration-300 ease-out top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center w-[50%] aspect-square rounded-full bg-gradient-to-r ${rouletteWheelNumbers.find((n) => n.Number == outcomeNumber)?.Color == "Red" ? 'from-red-600 via-[#e79494] to-red-600' : rouletteWheelNumbers.find((n) => n.Number == outcomeNumber)?.Color == "Black" ? 'from-gray-800 via-gray-600 to-gray-800' : 'from-[#00ff15] via-[#92f09a] to-[#00ff15]'} shadow-lg`}>
                                <div className="text-white text-4xl font-bold">{outcomeNumber}</div>
                                <div className="text-white text-sm mt-2 uppercase">{rouletteWheelNumbers.find((n) => n.Number == outcomeNumber)?.Color}</div>
                            </div>
                        </div>
                    </div>}
                    <div className="flex w-full justify-end relative">
                        {isMobile && < div className={`${!isWheeling && !autoBetting ? "w-[35%] left-[20%] top-[20%]" : "w-[80%] left-[50%] top-[50%]"} absolute  -translate-y-1/2 -translate-x-1/2  z-20  transition-all duration-300 aspect-square `} style={{ aspectRatio: "1/1", borderRadius: "50% 50% 0 0" }}>
                            <RouletteCanvas
                                outcomeNumber={outcomeNumber}
                                onAnimationEnd={() => setShowResult(true)} />
                            <div className={`absolute transition-opacity ${(showResult && isWheeling) ? "opacity-95" : "opacity-0"} duration-300 ease-out top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center w-[50%] aspect-[1/1] rounded-full bg-gradient-to-r ${rouletteWheelNumbers.find((n) => n.Number == outcomeNumber)?.Color == "Red" ? 'from-red-600 via-[#e79494] to-red-600' : rouletteWheelNumbers.find((n) => n.Number == outcomeNumber)?.Color == "Black" ? 'from-gray-800 via-gray-600 to-gray-800' : 'from-[#00ff15] via-[#92f09a] to-[#00ff15]'} shadow-lg`}>
                                <div className="text-white text-4xl font-bold">{outcomeNumber}</div>
                                <div className="text-white text-sm mt-2 uppercase">{rouletteWheelNumbers.find((n) => n.Number == outcomeNumber)?.Color}</div>
                            </div>
                        </div>}
        ...
        <div className={`flex  w-full ${isMobile ? "max-w-[400px] flex-col" : "max-w-[1300px]"}`}>
            <SettingBar maxBetDisable={true} fairness={
                <FairnessView gameId={"roulette"} privateSeed={serverSeed} privateHash={serverSeedHash} publicSeed={clientSeed} prev={true} >
